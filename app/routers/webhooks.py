...
def _handle_call_started(call_data):
    ...
    if not shop_result.data:
        return
    supabase.table("calls").insert({"shop_id": shop_result.data[0]["id"], "retell_call_id": retell_call_id, "caller_number": call_data.get("from_number"), "status": "in_progress"}).execute()
...
def _handle_call_ended(call_data):
    ...
    if call_result.data:
        ...
        if not shop_r.data: return
        shop = shop_r.data[0]
        r = supabase.table("calls").insert({"shop_id": shop["id"], "retell_call_id": retell_call_id, "caller_number": call_data.get("from_number"), "status": "completed"}).execute()
        call_record = r.data[0]; call_record["shops"] = shop
    shop = call_record.get("shops", {})
    call_id = call_record["id"]
    transcript_str = "".join(f"{e.get('role', '').capitalize()}: {e.get('content','')}\n" for e in call_data.get("transcript_object",[]))
    duration = int((call_data.get("end_timestamp",0) - call_data.get("start_timestamp",0)) / 1000) if call_data.get("end_timestamp") else None
    supabase.table("calls").update({"status": "completed", "transcript": transcript_str, "duration_seconds": duration}).eq("id", call_id).execute()
    if not transcript_str.strip(): return
    try:
        ext = openai_service.extract_appointment_from_transcript(transcript_str, shop.get("name","shop"))
        supabase.table("calls").update({"summary": ext.get("summary"), "appointment_booked": ext.get("appointment_booked",False)}).eq("id",call_id).execute()
        if ext.get("appointment_booked"):
            supabase.table("appointments").insert({"shop_id":shop["id"],"call_id":call_id,"customer_name":ext.get("customer_name"),"customer_phone":ext.get("customer_phone")or call_data.get("from_number"),"vehicle_info":ext.get("vehicle_info"),"service_requested":ext.get("service_requested"),"preferred_date":ext.get("preferred_date"),"preferred_time":ext.get("preferred_time")}).execute()
...
async def stripe_webhook(request: Request):
    ...
    try: event = stripe_service.construct_webhook_event(payload,sig)
    except Exception as e: raise HTTPException(status_code=400,detail=str(e))
    t = event["type"]; d = event["data"]["object"]
    if t == "checkout.session.completed":
        sid = d.get("metadata",{}).get("shop_id")
        if sid: supabase.table("shops").update({"stripe_subscription_id":d.get("subscription"),"stripe_customer_id":d.get("customer") ,"subscription_status":"active"}).eq("id",sid).execute()
